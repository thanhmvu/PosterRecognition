...
// l.side*l.side*l.n, thresh, boxes, probs, voc_names, voc_labels, 20
int updateCorrect(int num, float thresh, float **probs, int classes, int * labels, char * path, int correct){
		float max_prob = 0; float max_prob2 = 0;
		int max_class = -1; int max_class2 = -1;
		int j;
		for(j = 0; j < num; ++j){
				int classIdx = max_index(probs[j], classes);
				float prob = probs[j][classIdx];
				if(prob > thresh){
						if (prob> max_prob){
								max_prob = prob;
								max_class = labels[classIdx];
						} else if (prob> max_prob2) {
								max_prob2 = prob;
								max_class2 = labels[classIdx]; } } }
...
void validate_poster(char *cfgfile, char *weightfile, char * testImgs, int start_label)
{
    int class_labels[1000] = {}; // the maximum number of classes is hardcored to be 1000
	...
	int label;
    for(label = 0; label < classes; label += 1){ class_labels[label] = start_label + label; }
    ...
    for(i = nthreads; i < m+nthreads; i += nthreads){
        ...
        for(t = 0; t < nthreads && i+t-nthreads < m; ++t){
            ...
            correct = updateCorrect(side *side *l.n, thresh, probs, classes, class_labels, path, correct);
            total++;
...
//     float thresh = find_float_arg(argc, argv, "-thresh", .2);
	...
	int start_label = find_int_arg(argc, argv, "-sl", 0);
    if(0==strcmp(argv[2], "train")) train_poster(cfg, weights, filename, backup);
    else if(0==strcmp(argv[2], "valid")) validate_poster(cfg, weights, filename, start_label);
